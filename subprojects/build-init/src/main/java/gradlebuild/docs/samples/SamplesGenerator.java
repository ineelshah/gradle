...
public class SamplesGenerator {
    ...
    public static void generate(String type, Directory target, ProjectLayoutSetupRegistry projectLayoutSetupRegistry) {
        CompositeProjectInitDescriptor descriptor = (CompositeProjectInitDescriptor) projectLayoutSetupRegistry.get(type);

        String projectName = "demo";
        String subprojectName = descriptor.getComponentType().getDefaultProjectName();
        String packageName = descriptor.supportsPackage() ? projectName : null;
        BuildInitTestFramework testFramework = descriptor.getDefaultTestFramework();

        InitSettings groovyDslSettings = new InitSettings(projectName, subprojectName, BuildInitDsl.GROOVY, packageName, testFramework, target.dir("groovy"));
        InitSettings kotlinDslSettings = new InitSettings(projectName, subprojectName, BuildInitDsl.KOTLIN, packageName, testFramework, target.dir("kotlin"));

        List<String> comments = descriptor.generateWithExternalComments(groovyDslSettings);
        ...
        generateOutput(groovyDslSettings, descriptor, projectLayoutSetupRegistry);
    }
    ...
    private static void generateReadmeFragment(String templateFragment, InitSettings settings, List<String> comments,
        ...
        projectLayoutSetupRegistry.getTemplateOperationFactory().newTemplateOperation()
            ...
            .withBinding("subprojectName", settings.getSubprojectName())
    ...
    private static void generateOutput(InitSettings baseSettings, CompositeProjectInitDescriptor descriptor, ProjectLayoutSetupRegistry projectLayoutSetupRegistry) {
        String extraCompileJava = "> Task :" + baseSettings.getSubprojectName() + ":compileJava NO-SOURCE\n";
        String extraCompileTestJava = "> Task :" + baseSettings.getSubprojectName() + ":compileTestJava NO-SOURCE\n";
        if (descriptor.getLanguage().toString().equals("Java")) {
            extraCompileJava = "";
            extraCompileTestJava = "";
        }
        projectLayoutSetupRegistry.getTemplateOperationFactory().newTemplateOperation()
            .withTemplate("readme/build.out.template")
            .withTarget(baseSettings.getTarget().file("../tests/build.out").getAsFile())
            .withBinding("language", descriptor.getLanguage().toString())
            .withBinding("subprojectName", baseSettings.getSubprojectName())
